 ...
 * @author Kausik N
 */
public class Maze_RecursiveThreadedGen_BasicGUI extends javax.swing.JFrame {

    /**
     * Creates new form Maze_BasicGen_BasicGUI
     */
    public Maze_RecursiveThreadedGen_BasicGUI() {
        initComponents();
    }
...
//            }
    ...
    public class PathTree extends Thread
    {
        ...
        String id_recursion;
        PathTreeNode parent_recursion;
        int depth_recursion;
        
        PathTreeNode currentnode_recursion;
        public void run()  //GeneratePathTree_Recursive()
        {
            if(depth_recursion <= max_tree_depth)
            {
                ...
                {
                    PathTreeNode newNode = new PathTreeNode(id_recursion, 0, new PathTreeNode[max_branches], parent_recursion, false);
                    for(int i=0;i<max_branches;i++)
                    {
                        newNode.children[i] = null;
                    }
                    no_of_nodes = no_of_nodes + 1;
                    currentnode_recursion = newNode;
                }
                // Non Dead End
                for(int i=0;i<max_branches;i++)
                {
                ...
                PathTreeNode newNode = new PathTreeNode(id_recursion, no_of_children, new PathTreeNode[max_branches], parent_recursion, false);
                for(int i=0;i<max_branches;i++)
                {
                    ...
                    { 
                        ...
                        {
                            newNode.children[i] = GetRandomDestinationNode(id_recursion + "_" + i, 0);
                        }
                        else
                        {
                            System.out.println("Generating Depth " + depth_recursion);
                            id_recursion = id_recursion + "_" + i;
                            depth_recursion = depth_recursion + 1;
                            parent_recursion = newNode;
                            PathTree newPathTreeThread = new PathTree();
                            CopyClass(newPathTreeThread);
                            Thread t = newPathTreeThread;
                            t.start();
                            newNode.children[i] = newPathTreeThread.currentnode_recursion; //GeneratePathTree_Recursive();  // For normal child node
                        }
                    }
            ...
            currentnode_recursion = null;
            if(RootNode == null) RootNode = currentnode_recursion;
        }
...
//                    }
        ...
        void CopyClass(PathTree p)
        {
            ...
            p.id_recursion = id_recursion;
            p.depth_recursion = depth_recursion;
            p.currentnode_recursion = currentnode_recursion;
            p.parent_recursion = parent_recursion;
            p.RootNode = RootNode;
...
//  int no_of_final_nodes_param
    ...
    Maze.InitPathTreeParameters(3, 0.0f, 3, 1.0f, 0.0f, 1);
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Maze_RecursiveThreadedGen_BasicGUI().setVisible(true);
            }
        });
    }
